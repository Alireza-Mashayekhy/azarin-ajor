    ...
    <div className="relative w-full">
      <input
        ...
        className={cn(
          ...
          `${props.maxLength ? 'pr-10' : ''}`,
          className
        )}
        {...props}
      />
      {!!props.maxLength && (
        <span className="absolute top-1/2 right-0 -translate-y-1/2 text-[10px] text-muted-foreground px-2">
          {typeof props.value === 'string' || Array.isArray(props.value) ? props.value.length : 0}/
          {props.maxLength}
        </span>
      )}
    </div>
